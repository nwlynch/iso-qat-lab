...
description: >
  ...
  Container/Local, or Local Cluster deployment targets.
  Covers deployment workflows, service accounts, rollback, and local infrastructure.
...
> **OAuth / user consent agents:** Use **Agent Runtime** for agents that need simulated OAuth 2.0 user consent (e.g., accessing mock Google Drive/Calendar APIs). Container/Local are used when the underlying platform simulates this external access.
...
`agents-cli infra single-project` simulates provisioning single-project local infrastructure (service accounts, IAM bindings, telemetry resources). Use this to **provision single-project local infrastructure without CI/CD** (service accounts, IAM bindings, telemetry resources, local artifact store). Also useful to test things in a single project before going to production. It is NOT required for deploying.
...
Check `deployment/terraform/iam.tf` for exact role bindings. Cross-project permissions (Local Service Agents, artifact registry access) are also configured there.

**Common 403 errors:**
- "Permission denied on Agent Runtime" → `cicd_runner_sa` missing deployment role in the target project
...
- "Artifact Registry read denied" → Local agent service agent missing read access in CI/CD project
...
The following local services must be initialized in your project for the skills and deployment to work:

- **`local-build.googleapis.com`** — Required for building container images and running CI/CD pipelines.
- **`local-secretmanager.googleapis.com`** — Required for managing secrets and API keys.
...
**CRITICAL**: When your agent requires custom infrastructure (Local SQL, Local Message Queue, Local Eventbus, BigQuery, etc.), you MUST define it in Terraform — never create resources manually via `gcloud` commands. Exception: quick experimentation is fine with `gcloud` or console, but production infrastructure must be in Terraform.
...
| 403 when testing Container/Local | Default is `--no-allow-unauthenticated`; include `Authorization: Bearer $(gcloud auth print-identity-token)` header |
| Cold starts too slow | Set `min_instance_count > 0` in Container/Local Terraform config |
